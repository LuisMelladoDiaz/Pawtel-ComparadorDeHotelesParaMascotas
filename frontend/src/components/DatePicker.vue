...
<template>
  <div class="date-picker-container relative rounded-lg">
    ...
    <p
      class="text-[18px] text-pawtel-black absolute left-[35px] bottom-[7px] w-auto pointer-events-none whitespace-nowrap overflow-hidden text-ellipsis hidden md:flex">
      {{ formattedDate || 'Establece un rango de fechas' }}
    </p>
    <p
      class="text-[14px] text-pawtel-black absolute left-[35px] bottom-[10px] w-auto pointer-events-none whitespace-nowrap overflow-hidden text-ellipsis md:hidden">
      {{ formattedDate || 'Establece un rango de fechas' }}
    </p>
  </div>
</template>
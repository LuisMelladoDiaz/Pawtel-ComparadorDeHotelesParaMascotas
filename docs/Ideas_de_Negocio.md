# Ideas de Negocio

## Portada

**Ideas de Negocio**  
**Versión 1.0**  
**Fecha: 07/02/2025**  
**Autores: Luis Mellado Díaz**  

---

## Índice

1. [Tabla de versiones](#tabla-de-versiones)
2. [Introducción](#introducción)
3. [Ideas de Negocio](#ideas-de-negocio)

---

## Tabla de Versiones

| Versión | Fecha | Descripción de cambios | Autor |
|---------|------------|----------------------|-------|
| 1.0 | 07/02/2025 | Creación del documento | Luis Mellado Díaz |

---

## Introducción

El propósito de este documento es que los miembros del equipo puedan poner en conjunto sus ideas de negocio. Se ha propuesto que cada miembro busque entre 3 y 5 negocios de nicho, extravagantes y exóticos. Para cada negocio que encuentren, deben formular qué problemas tiene y cómo podrá solucionarse con un servicio software.

---

## Ideas de Negocio

### Andrés Martínez Reviriego

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Negocio 1] | [Problema identificado] | [Solución o comentario] |
| [Negocio 2] | [Problema identificado] | [Solución o comentario] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Claudio Cortés Carrasco

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Intercambio de Objetos de Coleccionista] | [Coleccionistas de objetos extraños (taxidermias, artefactos históricos peculiares, etc) no tienen un espacio seguro y confiable para comprar] | [Plataforma con verificación de autenticidad, subastas y foros de discusión] |
| [Banco Virtual de Leche Materna] | [Escasez de leche materna para bebés prematuros o con intolerancias, y poca coordinación entre donantes y hospitales] | [Plataforma para conectar donantes de leche materna con bancos y centros de salud] |
| [Alquiler de Equipamiento de Laboratorio Casero] | [Aficionados a la ciencia como los propios estudiantes quieren experimentar, pero adquirir equipos (microscopios, impresoras 3D, etc) es caro] | [Plataforma de préstamo/alquiler directo entre personas y laboratorios pequeños] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Daniel Flores De Francisco

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Canguros para niños y cuidadores | El hecho de que muchas familias no tengan con quién dejar  su hij@s es un problema. Tambien, las pesonas mayores o simplemente aquellas que estan solas. Necesito alguna forma de buscar gente de confianza | Plataforma que pone en contacto familiares con canguros/cuidadores/personas de compañía. Se podría innovar, pues ya existe alguna plataforma, por lo que hay mercado. No he visto una que recoja el cuidado de niños y personas mayores en una misma app. |
| Hoteles para mascotas | Necesito una forma de buscar techo para mi mascota cuando voy de viaje o simplemente no puedo cuidarla. Un comparador de precios tampoco estaría mal, así puedo ahorrarme dinero. | Plataforma para buscar alojamiento para mascotas de forma económica. No os imaginais, el negocio de los hoteles para mascotas, sobretodo perros. Es de pijos, pero es totalmente algo que se utiliza. En Córdoba al menos hay 7 residencia de mascotas, en Sevilla ni te cuento... Es algo innovador y de nicho. |
| Cocina privada | No te apetecería alguna vez que te cocinase un chef profesional? Y si voy al campo con mis amigos y quiero una paella, un paellero? Y si simplemente quiero ponerme en contácto de la mano de un chef para que me enseñe en mi propia casa.| Plataforma de chefs tanto privados, casuales y gente que quiere compartir su experiencia culinaria. Precio ajustable al consumidor. Tarifa fija por parte del chef + complementos: (comprar ingredientes, recoger la cocina, clase de cocina). Se cobraría comisión. Hay mercado pero no tantos competidores.|
| Lavado de coches | El simple hecho de ir a lavar el coche, es un impedimento para precisamente lavarlo. Muchas veces da pereza, o no tienes tiempo. Estaría bien que alguien lo llevase a lavar por mi. | Plataforma para lavado de coche a domicilio. Viene un tipo de alguna lavandería, o autonomo y te lo lleva a algun sitio de lavado que tienen las herramientas necesarias. El problema que le veo es que pasa si te roban el coche. Si nos podemos asegurar que esa persona sea trackeable, no estaría tan mal. |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### David González Martínez

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Restaurantes saludables | Las plataformas de reviews actuales están ideadas para el público general. No disponen de la información remotamente necesaria para las personas interesadas en comer saludable. Si quiero saber si en 'restaurante x' podré disfrutar de un alimento de calidad, natural y nutritivo, me es imposible. | Se pueden utilizar conceptos sociales y de gamificación. Se puede monetizar mediante acuerdos con restaurantes para publicidad. |
| Fishing Buddy/Group | Es complicado encontrar grupos o compañeros de pesca (o salidas). Actualmente, la información y organización de salidas está dispersa en varios sitios. | La solución es crear una app que reúna lo que los aficionados buscan en este sentido. Son adultos, por lo general, así que quizás se puede ver el tema de suscripción. En todo caso, se puede hacer tratos con tiendas relacionadas para incorporar anuncios (o incluso integrarlas) en la app. |
| Hiking Buddy/Group | Similar al anterior. | Similar al anterior |
| Recados para personas mayores | Las personas mayores suelen requerir de ayuda para distintos recados (hacer la compra en el supermercado, farmacia, limpieza, hacer x o hacer y). Es común que los hijos los ayuden o contraten a otros, pero esto suele ser complicado.  | App donde se puede gestionar todo esto. Se cobran comisiones. |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

*Nota*: el 2 y el 3 se pueden, probablemente, extender a otros nichos. Se puede buscar información sobre lo que la gente hace y busca para encontrar salidas o compañeros online.
### Fernando Castelló Sánchez

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Trabajos de una vez | Siempre se buscan personas para pequeños trabajos que no puedes solucionar tu pero para los que tampoco quieres/tienes que contratar a una empresa. | Similar al negocio 4 de David, pero no centrado en personas mayores si no en cualquiera que necesite ayuda para cosas como hacer la compra, pasear a los perros, etc. No se buscaría a alguien que le ayude por ejemplo 1 vez a la semana, sino casos más específicos (Ej: me voy un fin de semana por trabajo y necesito que alguien cuide a mis perros solo esos días) |
| Un sitio donde los usuarios pueden probar productos a cambio de reseñas verificadas con fotos y vídeos. | Muchas reseñas online son falsas o manipuladas. | Las empresas pueden mejorar así sus productos antes de lanzarlos al mercado y los usuarios ganan productos gratis. Nos llevaríamos comisión por parte de la empresa y posibles acuerdos con marcas. |
| App de seguimiento de objetos perdidos | Siempre pasa que pierdes las llaves, la cartera o cualquier otra cosa y la mayoría de veces no hay forma de rastrearlos. | Sería una red de usuarios que reporten los objetos que encuentren y conectarlos con los dueños. Se podrían usar etiquetas QR con identificación anónima para facilitar el contacto.
Ganaríamos dinero con la venta de las etiquetas QR, que serían para pegarlas en objetos importantes. Podríamos dar recompensas también a los usuarios que más objetos ayuden a recuperar, acuerdos con seguros de pérdida. |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Francisco Miguel Jiménez Morales

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Appartament, tu buscador de compañeros de piso | Encontrar un compañero de piso compatible contigo para poder pagar los altos alquileres | Esta ya la comentamos pero la pongo otra vez por si acaso. Le he dado un par de vueltas y me gustaría proponerla quizás como un Linkedln de compañeros de piso o asi, más centrado en contactar con un perfil en específico que centrarnos en comunidades o que chatteen por ahí, etc. Se usaría para buscar a alguien que cumpla con el perfil de compañero pero no desarrollaríamos funcionalidades inútiles que ya se usan en otra app de la que no se van a mover. Propongo crear un sistema semejante a los "me gusta" para crear los "match", aunque ya ahí lo dejo abierto a vuestras sugerencias. |
| Paperworkn't: El rincón del vago del papeleo | Dificultad de los autónomos para manejar a la vez plazos de entrega de documentos legales, cuando se publica una ayuda, la burocracia no es clara con los requerimientos y plazos. Nuestro público objetivo son autónomos porque estos están centrados en su trabajo y les es más costoso revisar de forma costosa la manera de pedir ayudas o entregar cierta documentación. Pese a que el público objetivo inicial son autónomos, también es aplicable toda persona no familiarizada con la burocracia. Un caso de uso extra es por ejemplo el papeleo de la herencia, te diría y calcularía claramente los impuestos y papeleos. | Ignorad el nombre, lo primero que se me ha ocurrido. Este proyecto se monetiza vendiendo directamente la aplicación a los clientes (15,99€ por ejemplo). Es un trabajo de interfaz principalmente, tiene que ser MUY CLARA; nos servimos de que el gobierno y sus webs son una basura. Contras: requiere un trabajo por nuestra parte brutal de información y burocracia para poblar la web y requeriría en un caso real constantes actualizaciones.  |
| Biblioteca Digital de Juegos de Mesa Imprimibles | Juegos de rol, cartas y tableros listos para imprimir y jugar. TODO GRATIUTO SACADO DE INTERNET | Modelo de negocio mediante la publicitación de tiendas tanto locales como empresas vendedoras (Fnac por ejemplo o algo así) de juegos de mesa|
| Tienda online reventa de material educativo | Unificar en una plataforma la reventa de dicho material. (Un libro de lengua de primero de la eso solo se usa un año y luego no tiene valor) | Le veo la pega de que para eso está wallapop pero si con varias personas pensando se innova, podría intentarse. No le veo mucho futuro pero la pongo por si acaso la verdad. |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Javier García Sebastián

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Exposición de coleccionismo | No hay un lugar virtual donde exponer colecciones de objetos raros | todos los comentarios están en el grupo |
| Encuentra tu parking | es muy complicado aparcar en lugares poblados | Te dice los parkings de tu zona cercana y los que tienen huecos libres, para que no tengas que ir como un loco buscando aparcamiento cada vez que sales con el coche |
| Letterboxd para fiestas | cuando vas a salir a sitios sobre todo por primera vez o para innovar, no sabes que te vas a encontrar en la mayoría de ocasiones. | se habló en clase pero esto podria ser una manera de que se dejasen reseñas que de vd la gente va a leer (no como las del google maps que nipa) y ademas podriamos implementar un sistema de valoración (precios, musica, ambiente, etc) |
| Aplicación para parroquias | no hay una digamos "red social" delimitada para los católicos que quieran ir a misa | Sistema de gestión para parroquias (agendamiento de misas, eventos, inscripción a cursos). tambien una lista de paroquias que impartan misa en x hora y en x sitio |
| un comparador de libros | no os pasa que cuando leeis un libro con una vibra que os gusta, quereis encontrar mas libros de distintos autores con una vibra similar? | podriamos implementar foros de Q&A, comparativas directas entre libros, incluso juntarnos con alguna vendedora estilo la casa del libro para actuar de portal de venta |

### Javier Ruiz Garrido

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Entradas discoteca | Tener que comprar las entradas teniendo que hablar con un rrpp e incluso teniendo que ir a recogerlas a algun sitio, o enterarte de las ofertas a través de historias de instagram.  | Crear un sitio de venta de las entradas. Podría ser para que los rrpp suban ahí sus ofertas y opción de comprarselas directamente o directamente las discotecas y que se compren ahí las entradas y tengas una wallet con un qr (que sería la mejor opción pero habría que contar con el okey de las discotecas). Se ganaría dinero con anuncios o incluso con una muy pequeña comisión por entrada vendida en nuestra plataforma.|
| [Negocio 2] | [Problema identificado] | [Solución o comentario] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Jorge Gómez de Tovar

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| Cuidado parques | La falta de monitoreo de parques y áreas naturales impide su buen mantenimiento. | App colaborativa donde los usuarios reportan problemas y mantenimiento necesario en parques. |
| Organizador de Documentos Inteligente | Desorden con archivos digitales. | Clasificación automática de documentos con búsqueda avanzada. |
| Comparador de Gasolina |No saber dónde está la gasolina más barata. | Muestra precios en gasolineras cercanas en tiempo real. |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Luis Mellado Díaz

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Negocio 1] | [Problema identificado] | [Solución o comentario] |
| [Negocio 2] | [Problema identificado] | [Solución o comentario] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Manuel Castillejo Vela

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Negocio 1] | [Problema identificado] | [Solución o comentario] |
| [Negocio 2] | [Problema identificado] | [Solución o comentario] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Rafael Castillo Cebolla

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Negocio 1] | [Problema identificado] | [Solución o comentario] |
| [Negocio 2] | [Problema identificado] | [Solución o comentario] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Sergio Trenado González

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Negocio 1] | [Problema identificado] | [Solución o comentario] |
| [Negocio 2] | [Problema identificado] | [Solución o comentario] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

### Yesica Garate Fuentes

| Negocio  | Problema a solucionar  | Propuestas o comentarios  |
|----------|-----------------------|---------------------------|
| [Aparcar un coche] | [A diario es complicado encontrar un lugar donde puedes aparcar tu coche] | [La idea que he pensado es hacer una red social (tipo twitter) donde las personas puedan dejar referencias en tiempo real de donde hay algun sitio donde se puede aparcar.] |
| [Marketplaces freelance] | [Desempleo y precariedad laboral] | [App que conecten a trabajadores independientes con proyectos puntuales, aprovechando el auge del trabajo por cuenta propia.] |
| [Negocio 3] | [Problema identificado] | [Solución o comentario] |
| [Negocio 4] | [Problema identificado] | [Solución o comentario] |
| [Negocio 5] | [Problema identificado] | [Solución o comentario] |

---


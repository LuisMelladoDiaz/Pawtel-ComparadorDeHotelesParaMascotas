...
from pawtel.room_types.models import RoomType, PetType
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not hotels.exists():
            self.stdout.write(self.style.ERROR('No hay hoteles en la base de datos. Ejecuta el seeder de hoteles primero.'))
            return
        ...
        for hotel in hotels:
            for room_data in room_types:
                ...
                self.stdout.write(self.style.SUCCESS(f'Habitación {room.name} creada en {hotel.name}'))

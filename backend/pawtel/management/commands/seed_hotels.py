...
class Command(BaseCommand):
    ...
    def create_deterministic_hotels(self):
        ...
        for hotel_data in deterministic_hotels:
            owner = HotelOwner.objects.filter(
                user__username=hotel_data["owner_username"]
            ).first()
            if not owner:
                self.stdout.write(
                    self.style.ERROR(
                        f"HotelOwner {hotel_data['owner_username']} no encontrado."
                    )
                )
                continue
            ...
            if not Hotel.objects.filter(name=hotel_data["name"]).exists():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created Deterministic Hotel: {hotel_data['name']}"
                    )
                )
            else:
                self.stdout.write(
    ...
    def create_random_hotels(self, num_random):
        ...
        nombres_hoteles_mascotas = [
            "Residencia El Rincón del Can",
            "Alojamiento Patas y Colas",
            "Hogar de Mascotas Felices",
            "Residencia Peludos",
            "Refugio Gatuno",
            "Alojamiento Paraíso Animal",
            "Residencia Amigo Fiel",
            "Hogar Colmillos",
            "Refugio Huellas",
            "Alojamiento Mi Mejor Amigo",
            "Residencia Cachorro Feliz",
            "Refugio Bigotes",
            "Alojamiento Patitas",
            "Residencia Colas Alegres",
            "Hogar Mascota Real",
            "Residencia Paws & Relax",
            "Alojamiento Happy Tails",
            "Refugio Furry Friends",
            "Residencia Wagging Tails",
            "Alojamiento Purrfect Stay",
            "Refugio Bark Avenue",
            "Residencia Whisker Haven",
            "Alojamiento Pawsome Place",
            "Hogar Tail Wagger Inn",
            "Refugio Feline Paradise",
            "Alojamiento Doggy Retreat",
            "Residencia Kitty Corner",
            "Refugio Canine Castle",
            "Alojamiento Meow Manor",
            "Residencia Puppy Palace",
            "Hogar Whisker Lodge",
            "Refugio Barkingham Palace",
            "Alojamiento Paws Inn",
            "Residencia Furry Retreat",
            "Hogar Tail Haven",
            "Alojamiento Purr Palace",
        ]

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not room_types:
            self.stdout.write(
                self.style.ERROR("No RoomTypes found. Run seed_room_types first.")
            )
            return
        ...
        for _ in range(20):  # Crear 20 habitaciones
            ...
            room = Room.objects.create(name=nombre_habitacion, room_type=room_type)
            self.stdout.write(
                self.style.SUCCESS(f"Created Room: {room.name} ({room_type.name})")
            )

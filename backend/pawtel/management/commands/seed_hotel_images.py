...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not hotels:
            self.stdout.write(self.style.ERROR("No hotels found. Please run the seed for hotels first."))
            return
    ...
    def create_hotel_images(self, hotels, num_images_per_hotel):
        for hotel in hotels:
            for _ in range(num_images_per_hotel):
                ...
                self.stdout.write(self.style.SUCCESS(f"Created image for Hotel: {hotel.name}"))
    ...
    def generate_image(self):
        ...
        img_byte_arr.seek(0)  
        image_file = InMemoryUploadedFile(img_byte_arr, None, 'hotel_image.jpg', 'image/jpeg', img_byte_arr.tell(), None)
        return image_file
